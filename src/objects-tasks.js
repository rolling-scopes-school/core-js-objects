...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    keys = keys.concat(Object.entries(objects[i]).map((val) => val[0]));
  }
  for (let j = 0; j < keys.length; j += 1) {
    const key = keys[j];
    result[key] = 0;
    for (let k = 0; k < objects.length; k += 1) {
      result[key] += objects[k][key] || 0;
    }
  }
  return result;
}
...
function removeProperties(obj, keys) {
  const resultingObj = obj;
  for (let i = 0; i < keys.length; i += 1) {
    delete resultingObj[keys[i]];
  }
  return resultingObj;
}
...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < objectEntries.length; i += 1) {
    ...
    for (let j = 0; j < objectEntries[i][1].length; j += 1) {
      result[objectEntries[i][1][j]] = key;
    }
  }
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] - 25 - change <= 0) {
      change += 25 - (queue[i] - 25);
    } else {
      return false;
    }
  }
