...
class SelectorBuilder {
  constructor(selector = '', rank = 0) {
    this.selector = selector;
    this.rank = rank;
  }

  element(value) {
    return this.create('element', value, 1);
  }

  id(value) {
    return this.create('id', value, 2, '#');
  }

  class(value) {
    return this.create('class', value, 3, '.');
  }

  attr(value) {
    return this.create('attr', value, 4, '[', ']');
  }

  pseudoClass(value) {
    return this.create('pseudoClass', value, 5, ':');
  }

  pseudoElement(value) {
    return this.create('pseudoElement', value, 6, '::');
  }

  combine(selector1, combinator, selector2) {
    this.selector = `${selector1.stringify()} ${combinator} ${selector2.stringify()}`;
    return new SelectorBuilder(this.selector);
  }

  create(type, value, rank, prefix = '', suffix = '') {
    if (this[type]) {
      throw new Error(
        `${
          type.charAt(0).toUpperCase() + type.slice(1)
        } should not occur more than one time inside the selector`
      );
    }
    if (rank < this.rank) {
      throw new Error(
    ...
    const selector = this.selector + prefix + value + suffix;
    return new SelectorBuilder(selector, rank, { ...this, [type]: true });
  }
